/**
 * Données des zones d'intervention — Serrurerie Roland
 * Siège : 60 rue Racine, 69100 Villeurbanne (intervention Lyon + Grand Lyon)
 *
 * Objectif SEO : une landing page par zone, avec un contenu UNIQUE.
 * Le champ `angleSerrurier` + `caractere` + `typeBati` servent à faire varier
 * la rédaction d'une page à l'autre (éviter le duplicate content).
 *
 * URL cible : /zones/serrurier-{slug}
 * Ex : /zones/serrurier-lyon-3, /zones/serrurier-villeurbanne
 */

export type ZoneType = "arrondissement" | "commune";

/**
 * Secteur géographique RÉEL de la zone — sert à ne jamais présenter une
 * commune hors métropole (Nord-Isère, Vienne...) comme faisant partie du
 * "Grand Lyon". Seule Chassieu, réellement membre de la Métropole de Lyon,
 * est classée "grand-lyon" parmi les communes ajoutées hors agglomération.
 *   lyon             : les 9 arrondissements de Lyon
 *   grand-lyon        : communes membres de la Métropole de Lyon
 *   est-nord-isere    : communes de l'Est lyonnais / Nord-Isère (hors métropole)
 *   sud-lyonnais      : communes du sud lyonnais (hors métropole)
 *   secteur-vienne    : communes du secteur de Vienne (hors métropole)
 */
export type SecteurGeo =
  | "lyon"
  | "grand-lyon"
  | "est-nord-isere"
  | "sud-lyonnais"
  | "secteur-vienne";

export interface Zone {
  /** Identifiant d'URL. URL finale = /zones/serrurier-{slug} */
  slug: string;
  /** Nom court affiché (ex: "Lyon 3e", "Villeurbanne") */
  nom: string;
  /** Nom complet pour les titres/H1 (ex: "Lyon 3ᵉ arrondissement") */
  nomComplet: string;
  type: ZoneType;
  /** Secteur géographique réel — voir `SecteurGeo`. Ne pas confondre avec `secteur` (texte libre éditorial). */
  secteurGeo: SecteurGeo;
  codePostal: string;
  /** Situation géo dans l'agglo, pour varier les intros */
  secteur: string;
  /** Quartiers rattachés — à citer pour ancrer le contenu localement */
  quartiers: string[];
  /** Points de repère connus — renforcent la pertinence locale */
  pointsDeRepere: string[];
  /** Caractère urbain du secteur (ambiance, densité) */
  caractere: string;
  /** Type de bâti dominant — oriente le vocabulaire technique */
  typeBati: string;
  /** Angle serrurier spécifique : le cœur de l'unicité éditoriale de la page */
  angleSerrurier: string;
  /** Slugs des zones limitrophes, pour le maillage interne */
  limitrophes: string[];
  /** true pour la zone où se trouve le siège (à mettre en avant) */
  siege?: boolean;
  /**
   * Contenu rédactionnel UNIQUE écrit à la main, pour enrichir en priorité
   * les zones stratégiques (les 9 arrondissements + Villeurbanne) et renforcer
   * l'unicité SEO au-delà de ce que génèrent déjà `angleSerrurier`/`quartiers`.
   * Chaque entrée = un paragraphe. Optionnel : si absent, la page compose son
   * contenu à partir des autres champs (aucune page ne reste vide).
   * // À COMPLÉTER PAR LE CLIENT (zone par zone, quand le temps le permet)
   */
  texteUnique?: string[];
  /**
   * Slugs des services les plus pertinents pour cette zone (maillage interne
   * ciblé, page zone → services). Optionnel : à défaut,
   * `getRelevantServicesForZone()` (@/lib/maillage) déduit les services
   * pertinents à partir des mots-clés retrouvés dans `angleSerrurier`.
   */
  servicesPertinents?: string[];
}

export const zones: Zone[] = [
  // ─────────────────────────────  ARRONDISSEMENTS DE LYON  ─────────────────────────────
  {
    slug: "lyon-1",
    nom: "Lyon 1er",
    nomComplet: "Lyon 1ᵉʳ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69001",
    secteur: "Centre / bas des pentes de la Croix-Rousse",
    quartiers: ["Terreaux", "Pentes de la Croix-Rousse", "Saint-Vincent", "Cordeliers"],
    pointsDeRepere: ["Place des Terreaux", "Hôtel de Ville", "Opéra de Lyon", "Musée des Beaux-Arts", "Place Sathonay"],
    caractere: "quartier historique animé, ruelles en pente et vie nocturne dense",
    typeBati: "immeubles anciens d'époque canut, nombreux petits commerces et copropriétés",
    angleSerrurier:
      "Bâti ancien avec des portes et serrures parfois d'origine : beaucoup de demandes de remplacement de serrures vétustes, d'ouverture de porte claquée dans les immeubles sans concierge, et de sécurisation de commerces en rez-de-chaussée.",
    limitrophes: ["lyon-2", "lyon-4", "lyon-6"],
  },
  {
    slug: "lyon-2",
    nom: "Lyon 2e",
    nomComplet: "Lyon 2ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69002",
    secteur: "Presqu'île, du centre à la Confluence",
    quartiers: ["Bellecour", "Ainay", "Cordeliers", "Perrache", "Confluence"],
    pointsDeRepere: ["Place Bellecour", "Rue de la République", "Musée des Confluences", "Gare de Perrache", "Centre commercial Confluence"],
    caractere: "cœur commerçant de Lyon et quartier neuf de la Confluence",
    typeBati: "immeubles bourgeois haussmanniens au nord, résidences et bureaux récents à la Confluence",
    angleSerrurier:
      "Contraste entre serrures anciennes des immeubles bourgeois d'Ainay/Bellecour et serrures multipoints et contrôles d'accès des résidences récentes de la Confluence : interventions autant en dépannage classique qu'en maintenance d'accès sécurisés.",
    limitrophes: ["lyon-1", "lyon-5", "lyon-7"],
  },
  {
    slug: "lyon-3",
    nom: "Lyon 3e",
    nomComplet: "Lyon 3ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69003",
    secteur: "Est du centre, autour de la Part-Dieu",
    quartiers: ["Part-Dieu", "Préfecture", "Montchat", "Sans-Souci", "Dauphiné"],
    pointsDeRepere: ["Gare de la Part-Dieu", "Centre commercial Part-Dieu", "Tour Part-Dieu (le Crayon)", "Halles Paul Bocuse", "Préfecture du Rhône"],
    caractere: "arrondissement le plus peuplé, quartier d'affaires très fréquenté",
    typeBati: "grands ensembles tertiaires et résidentiels près de la Part-Dieu, maisons de ville à Montchat",
    angleSerrurier:
      "Forte densité résidentielle et tertiaire : dépannages urgents fréquents (portes claquées, clés perdues) et sécurisation de locaux professionnels autour de la Part-Dieu, avec un secteur pavillonnaire à Montchat demandant plutôt du blindage et des serrures haute sécurité.",
    limitrophes: ["lyon-6", "lyon-7", "lyon-8", "villeurbanne", "bron"],
  },
  {
    slug: "lyon-4",
    nom: "Lyon 4e",
    nomComplet: "Lyon 4ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69004",
    secteur: "Plateau de la Croix-Rousse",
    quartiers: ["Plateau de la Croix-Rousse", "Serin", "Gros Caillou"],
    pointsDeRepere: ["Boulevard de la Croix-Rousse", "Marché de la Croix-Rousse", "Gros Caillou", "Place de la Croix-Rousse"],
    caractere: "esprit village, familial et convivial, marchés réputés",
    typeBati: "immeubles anciens de canuts, copropriétés et maisons de ville en pente",
    angleSerrurier:
      "Immeubles anciens aux entrées étroites et portes d'époque : beaucoup de remplacements de serrures usées, de sécurisation d'appartements familiaux et de dépannages après tentative d'effraction sur du bâti ancien.",
    limitrophes: ["lyon-1", "lyon-9", "caluire-et-cuire"],
  },
  {
    slug: "lyon-5",
    nom: "Lyon 5e",
    nomComplet: "Lyon 5ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69005",
    secteur: "Ouest, du Vieux Lyon aux collines",
    quartiers: ["Vieux Lyon (Saint-Jean, Saint-Paul, Saint-Georges)", "Fourvière", "Point du Jour", "Ménival", "Champvert"],
    pointsDeRepere: ["Basilique de Fourvière", "Cathédrale Saint-Jean", "Vieux Lyon et ses traboules", "Théâtres romains de Fourvière"],
    caractere: "secteur patrimonial classé UNESCO et quartiers résidentiels sur les hauteurs",
    typeBati: "immeubles Renaissance et portes anciennes dans le Vieux Lyon, pavillons et résidences sur les collines",
    angleSerrurier:
      "Le Vieux Lyon impose un travail soigné sur des portes anciennes et parfois classées (respect du bâti, serrures adaptées), tandis que les quartiers de collines (Point du Jour, Champvert) demandent surtout du blindage et de la serrure haute sécurité pour maisons individuelles.",
    limitrophes: ["lyon-2", "lyon-9", "sainte-foy-les-lyon", "tassin-la-demi-lune"],
  },
  {
    slug: "lyon-6",
    nom: "Lyon 6e",
    nomComplet: "Lyon 6ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69006",
    secteur: "Nord-est, quartier des Brotteaux",
    quartiers: ["Brotteaux", "Foch", "Masséna", "Tête d'Or"],
    pointsDeRepere: ["Parc de la Tête d'Or", "Les Brotteaux (ancienne gare)", "Cité Internationale", "Boulevard des Belges"],
    caractere: "arrondissement résidentiel chic et bourgeois",
    typeBati: "immeubles bourgeois de standing, appartements familiaux haut de gamme",
    angleSerrurier:
      "Clientèle attentive à la sécurité et à la discrétion : demande importante en portes blindées, serrures multipoints certifiées A2P et sécurisation d'appartements de standing, avec exigence d'un travail propre et sans dégât sur des portes de valeur.",
    limitrophes: ["lyon-1", "lyon-3", "villeurbanne", "caluire-et-cuire"],
  },
  {
    slug: "lyon-7",
    nom: "Lyon 7e",
    nomComplet: "Lyon 7ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69007",
    secteur: "Sud-est, de la Guillotière à Gerland",
    quartiers: ["Guillotière", "Jean Macé", "Gerland", "Berthelot", "Université"],
    pointsDeRepere: ["Stade de Gerland", "Halle Tony Garnier", "Place Jean Macé", "campus universitaires et ENS"],
    caractere: "quartier étudiant, populaire et en pleine mutation",
    typeBati: "mix d'immeubles anciens, de logements étudiants et de résidences neuves à Gerland",
    angleSerrurier:
      "Forte rotation locative (étudiants, jeunes actifs) : nombreux changements de barillet entre deux locations, ouvertures de portes après perte de clés, et sécurisation de studios ; côté Gerland, davantage de locaux professionnels et de résidences récentes à équiper.",
    limitrophes: ["lyon-2", "lyon-3", "lyon-8"],
  },
  {
    slug: "lyon-8",
    nom: "Lyon 8e",
    nomComplet: "Lyon 8ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69008",
    secteur: "Sud-est résidentiel",
    quartiers: ["Monplaisir", "États-Unis", "Bachut", "Grand Trou", "Laënnec", "Mermoz"],
    pointsDeRepere: ["Institut Lumière (Monplaisir)", "Place du Bachut", "Hôpital Édouard Herriot", "quartier des États-Unis (Tony Garnier)"],
    caractere: "arrondissement résidentiel et familial en développement",
    typeBati: "immeubles des années 1930 à aujourd'hui, copropriétés et quelques maisons de ville",
    angleSerrurier:
      "Habitat familial dense : demandes régulières de remplacement de serrures, de blindage de porte d'appartement et de dépannage d'urgence, avec un tissu de commerces de proximité (Bachut, Monplaisir) à sécuriser.",
    limitrophes: ["lyon-3", "lyon-7", "bron", "venissieux"],
  },
  {
    slug: "lyon-9",
    nom: "Lyon 9e",
    nomComplet: "Lyon 9ᵉ arrondissement",
    type: "arrondissement",
    secteurGeo: "lyon",
    codePostal: "69009",
    secteur: "Nord-ouest, le long de la Saône",
    quartiers: ["Vaise", "Gorge de Loup", "Saint-Rambert", "Île Barbe", "La Duchère"],
    pointsDeRepere: ["Gare de Vaise", "Île Barbe", "quartier de La Duchère", "pôle multimodal Gorge de Loup"],
    caractere: "secteur mêlant zones résidentielles, tertiaires et rénovation urbaine",
    typeBati: "résidences récentes à Vaise, grands ensembles rénovés à La Duchère, maisons à Saint-Rambert",
    angleSerrurier:
      "Diversité de bâti : immeubles récents et tertiaires autour de Vaise/Gorge de Loup (contrôles d'accès, serrures multipoints) et pavillons de Saint-Rambert à protéger par du blindage, avec des interventions d'urgence courantes dans les copropriétés.",
    limitrophes: ["lyon-4", "lyon-5", "caluire-et-cuire", "ecully"],
  },

  // ─────────────────────────────  COMMUNES DU GRAND LYON  ─────────────────────────────
  {
    slug: "villeurbanne",
    nom: "Villeurbanne",
    nomComplet: "Villeurbanne",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69100",
    secteur: "Est de Lyon, commune limitrophe (siège de l'entreprise)",
    quartiers: ["Gratte-Ciel", "Charpennes", "Cusset", "Tonkin", "Les Brosses", "Croix-Luizet", "Buers", "Saint-Jean"],
    pointsDeRepere: ["Gratte-Ciel", "Théâtre National Populaire (TNP)", "Campus de La Doua", "Le Rize", "Place Charpennes"],
    caractere: "deuxième ville de la métropole, dense, résidentielle et étudiante",
    typeBati: "immeubles Art déco des Gratte-Ciel, copropriétés variées et logements étudiants près de La Doua",
    angleSerrurier:
      "C'est notre commune d'implantation : intervention ultra-rapide sur tout Villeurbanne, du dépannage d'urgence dans les copropriétés des Gratte-Ciel et de Charpennes au changement de barillet fréquent près du campus de La Doua, en passant par le blindage de porte dans les quartiers résidentiels.",
    limitrophes: ["lyon-3", "lyon-6", "vaulx-en-velin", "bron", "caluire-et-cuire"],
    siege: true,
  },
  {
    slug: "caluire-et-cuire",
    nom: "Caluire-et-Cuire",
    nomComplet: "Caluire-et-Cuire",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69300",
    secteur: "Nord de Lyon, entre Rhône et Saône",
    quartiers: ["Caluire centre", "Cuire", "Montessuy", "Bissardon", "Saint-Clair", "Vassieux"],
    pointsDeRepere: ["bords de Saône et du Rhône", "Île Barbe (proximité)", "quartier de Saint-Clair"],
    caractere: "résidentielle aisée, pavillonnaire et calme",
    typeBati: "maisons individuelles et petites copropriétés résidentielles",
    angleSerrurier:
      "Habitat majoritairement pavillonnaire : forte demande de portes blindées, de serrures haute sécurité et de sécurisation de maisons individuelles, ainsi que de portails et portes de garage motorisés.",
    limitrophes: ["lyon-4", "lyon-6", "lyon-9", "villeurbanne"],
  },
  {
    slug: "bron",
    nom: "Bron",
    nomComplet: "Bron",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69500",
    secteur: "Est de Lyon",
    quartiers: ["Bron centre", "Terraillon", "Parilly", "UMV"],
    pointsDeRepere: ["Aéroport de Bron", "Parc de Parilly", "hôpitaux Est (Pinel, neurologique)"],
    caractere: "de l'est mêlant résidentiel et zones d'activité",
    typeBati: "mix de copropriétés, de pavillons et de locaux d'activité",
    angleSerrurier:
      "Habitat varié et présence de zones d'activité : dépannages résidentiels courants et sécurisation de locaux professionnels et commerciaux (rideaux métalliques, serrures renforcées).",
    limitrophes: ["lyon-3", "lyon-8", "villeurbanne", "venissieux", "saint-priest"],
  },
  {
    slug: "venissieux",
    nom: "Vénissieux",
    nomComplet: "Vénissieux",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69200",
    secteur: "Sud-est de Lyon",
    quartiers: ["Vénissieux centre", "Les Minguettes", "Parilly", "Moulin à Vent"],
    pointsDeRepere: ["Marché de Vénissieux", "Parc de Parilly", "zones industrielles sud-est"],
    caractere: "populaire et dense, résidentielle et industrielle",
    typeBati: "grands ensembles, copropriétés et importantes zones d'activité",
    angleSerrurier:
      "Dépannages d'urgence fréquents dans l'habitat collectif dense, et forte demande de sécurisation (rideaux métalliques, portes blindées, serrures anti-effraction) pour les commerces et locaux professionnels des zones d'activité.",
    limitrophes: ["lyon-8", "bron", "saint-priest"],
  },
  {
    slug: "vaulx-en-velin",
    nom: "Vaulx-en-Velin",
    nomComplet: "Vaulx-en-Velin",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69120",
    secteur: "Est de Lyon, le long du canal de Jonage",
    quartiers: ["Le Village", "Mas du Taureau", "La Grappinière", "Carré de Soie"],
    pointsDeRepere: ["Planétarium de Vaulx-en-Velin", "pôle du Carré de Soie", "canal de Jonage"],
    caractere: "en rénovation urbaine, entre logements collectifs et secteurs pavillonnaires",
    typeBati: "grands ensembles rénovés, pavillons au Village et résidences neuves au Carré de Soie",
    angleSerrurier:
      "Interventions d'urgence dans l'habitat collectif et sécurisation des résidences neuves du Carré de Soie (contrôles d'accès, serrures multipoints), avec du blindage pour les pavillons du Village.",
    limitrophes: ["villeurbanne", "decines-charpieu", "bron"],
  },
  {
    slug: "saint-priest",
    nom: "Saint-Priest",
    nomComplet: "Saint-Priest",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69800",
    secteur: "Sud-est de la métropole",
    quartiers: ["Saint-Priest centre", "Bel Air", "Manissieux", "Revaison"],
    pointsDeRepere: ["Château de Saint-Priest", "Parc technologique de Lyon", "zones industrielles est"],
    caractere: "pavillonnaire dotée d'importantes zones d'activité",
    typeBati: "maisons individuelles, lotissements récents et parcs d'activité",
    angleSerrurier:
      "Prédominance de maisons individuelles et de lotissements : blindage, portails et portes de garage motorisés, serrures haute sécurité, complétés par la sécurisation des entreprises du parc technologique.",
    limitrophes: ["bron", "venissieux", "meyzieu"],
  },
  {
    slug: "ecully",
    nom: "Écully",
    nomComplet: "Écully",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69130",
    secteur: "Ouest de Lyon",
    quartiers: ["Écully centre", "Le Pérollier", "Charrière Blanche"],
    pointsDeRepere: ["emlyon business school", "École Centrale de Lyon", "quartiers résidentiels boisés"],
    caractere: "résidentielle cossue de l'ouest lyonnais",
    typeBati: "maisons individuelles de standing et résidences haut de gamme",
    angleSerrurier:
      "Clientèle exigeante en sécurité : portes blindées, serrures certifiées A2P, coffres et sécurisation de propriétés individuelles, avec un travail soigné attendu sur des menuiseries de qualité.",
    limitrophes: ["lyon-9", "tassin-la-demi-lune"],
  },
  {
    slug: "tassin-la-demi-lune",
    nom: "Tassin-la-Demi-Lune",
    nomComplet: "Tassin-la-Demi-Lune",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69160",
    secteur: "Ouest de Lyon",
    quartiers: ["La Demi-Lune", "Le Méridien", "Alaï"],
    pointsDeRepere: ["Horloge de Tassin", "vallon des Planches", "axes vers l'ouest lyonnais"],
    caractere: "résidentielle familiale de l'ouest",
    typeBati: "maisons individuelles et résidences récentes",
    angleSerrurier:
      "Habitat pavillonnaire et petites copropriétés : demande soutenue de blindage, de serrures haute sécurité et de motorisation de portails/portes de garage pour les maisons familiales.",
    limitrophes: ["lyon-5", "ecully", "sainte-foy-les-lyon"],
  },
  {
    slug: "oullins",
    nom: "Oullins",
    nomComplet: "Oullins",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69600",
    secteur: "Sud de Lyon, rive droite du Rhône",
    quartiers: ["Oullins centre", "La Saulaie", "Le Golf"],
    pointsDeRepere: ["terminus de la ligne B (Oullins Gare)", "bords de Saône", "Grande Rue commerçante"],
    caractere: "en développement, bien desservie par le métro",
    typeBati: "mix d'immeubles anciens du centre, de copropriétés et de programmes neufs",
    angleSerrurier:
      "Centre-ville commerçant et arrivée du métro dynamisent l'habitat : dépannages d'urgence dans les copropriétés, changements de serrures dans l'ancien et équipement des résidences neuves.",
    limitrophes: ["lyon-2", "sainte-foy-les-lyon", "saint-genis-laval"],
  },
  {
    slug: "sainte-foy-les-lyon",
    nom: "Sainte-Foy-lès-Lyon",
    nomComplet: "Sainte-Foy-lès-Lyon",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69110",
    secteur: "Sud-ouest, sur les coteaux",
    quartiers: ["Sainte-Foy centre", "Le Plan du Loup", "La Gravière", "Beaunant"],
    pointsDeRepere: ["coteaux avec vue sur Lyon", "centre-bourg résidentiel", "proximité du Vieux Lyon"],
    caractere: "résidentielle aisée perchée sur les collines",
    typeBati: "maisons individuelles et résidences de standing",
    angleSerrurier:
      "Habitat pavillonnaire cossu sur les hauteurs : forte demande de portes blindées, serrures haute sécurité, portails et sécurisation périmétrique des propriétés.",
    limitrophes: ["lyon-5", "tassin-la-demi-lune", "oullins", "saint-genis-laval"],
  },
  {
    slug: "saint-genis-laval",
    nom: "Saint-Genis-Laval",
    nomComplet: "Saint-Genis-Laval",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69230",
    secteur: "Sud-ouest de la métropole",
    quartiers: ["Saint-Genis centre", "Beaunant", "Basses Barolles"],
    pointsDeRepere: ["Observatoire de Lyon", "hôpitaux sud", "centre-bourg"],
    caractere: "résidentielle du sud-ouest, en extension du métro",
    typeBati: "maisons individuelles, lotissements et petites résidences",
    angleSerrurier:
      "Zone majoritairement pavillonnaire : blindage, serrures haute sécurité, portails et portes de garage motorisés pour maisons familiales, avec du dépannage courant dans les résidences.",
    limitrophes: ["oullins", "sainte-foy-les-lyon"],
  },
  {
    slug: "decines-charpieu",
    nom: "Décines-Charpieu",
    nomComplet: "Décines-Charpieu",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69150",
    secteur: "Est de la métropole",
    quartiers: ["Décines centre", "Le Sixième", "Grand Large"],
    pointsDeRepere: ["Groupama Stadium (Parc OL)", "plan d'eau du Grand Large", "canal de Jonage"],
    caractere: "en forte croissance, pavillonnaire et sportive",
    typeBati: "maisons individuelles, lotissements récents et nouvelles résidences",
    angleSerrurier:
      "Croissance résidentielle rapide : équipement de maisons et résidences neuves (serrures multipoints, portails, portes de garage) et blindage des pavillons, avec du dépannage d'urgence dans les lotissements.",
    limitrophes: ["vaulx-en-velin", "meyzieu"],
  },
  {
    slug: "meyzieu",
    nom: "Meyzieu",
    nomComplet: "Meyzieu",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69330",
    secteur: "Est de la métropole, près du Grand Parc",
    quartiers: ["Meyzieu centre", "Le Carreau", "zone industrielle"],
    pointsDeRepere: ["Grand Large", "proximité du Parc de Miribel-Jonage", "tramway T3 / Rhônexpress"],
    caractere: "pavillonnaire en expansion à l'est de l'agglomération",
    typeBati: "maisons individuelles, lotissements et zone industrielle",
    angleSerrurier:
      "Tissu pavillonnaire et zone industrielle : blindage et serrures haute sécurité pour les maisons, sécurisation des entreprises et locaux (rideaux métalliques, serrures renforcées) sur la ZI.",
    limitrophes: ["decines-charpieu", "saint-priest"],
  },
  {
    slug: "rillieux-la-pape",
    nom: "Rillieux-la-Pape",
    nomComplet: "Rillieux-la-Pape",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69140",
    secteur: "Nord-est de Lyon, sur le plateau",
    quartiers: ["Rillieux centre", "La Ville Nouvelle", "Crépieux-la-Pape", "Vancia", "Les Alagniers"],
    pointsDeRepere: ["plateau de Rillieux", "canal de Miribel", "zone commerciale"],
    caractere: "du nord-est mêlant grands ensembles rénovés et secteurs pavillonnaires",
    typeBati: "habitat collectif à la Ville Nouvelle, pavillons à Crépieux et Vancia",
    angleSerrurier: "Contraste marqué : dépannages d'urgence fréquents dans l'habitat collectif de la Ville Nouvelle, et blindage, portails et serrures haute sécurité pour les pavillons de Crépieux-la-Pape et Vancia.",
    limitrophes: ["caluire-et-cuire", "vaulx-en-velin"],
  },
  {
    slug: "saint-fons",
    nom: "Saint-Fons",
    nomComplet: "Saint-Fons",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69190",
    secteur: "Sud de Lyon, le long du Rhône",
    quartiers: ["Saint-Fons centre", "Les Clochettes", "Arsenal-Carnot"],
    pointsDeRepere: ["bords du Rhône", "Vallée de la Chimie", "gare de Saint-Fons"],
    caractere: "populaire et industrielle du sud",
    typeBati: "habitat collectif dense et importantes zones industrielles",
    angleSerrurier: "Forte présence industrielle (sécurisation de sites et locaux, rideaux métalliques, serrures renforcées) combinée à des dépannages d'urgence réguliers dans l'habitat collectif du centre et des Clochettes.",
    limitrophes: ["venissieux", "lyon-7", "lyon-8", "feyzin"],
  },
  {
    slug: "givors",
    nom: "Givors",
    nomComplet: "Givors",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69700",
    secteur: "Extrême sud de la Métropole, au confluent Rhône-Gier",
    quartiers: ["Givors centre", "Les Vernes", "Bans", "Canal"],
    pointsDeRepere: ["confluent Rhône-Gier", "anciennes verreries", "gare de Givors"],
    caractere: "ancienne ville industrielle du sud, en rénovation",
    typeBati: "centre ancien, grands ensembles et secteurs pavillonnaires",
    angleSerrurier: "Centre ancien aux portes et serrures vétustes (remplacements fréquents) et grands ensembles rénovés (dépannages d'urgence), avec une demande de sécurisation pour les commerces du centre-ville.",
    limitrophes: ["saint-genis-laval", "feyzin"],
  },
  {
    slug: "francheville",
    nom: "Francheville",
    nomComplet: "Francheville",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69340",
    secteur: "Ouest de Lyon, sur les coteaux",
    quartiers: ["Francheville le Bas", "Francheville le Haut", "Bel Air", "Taffignon"],
    pointsDeRepere: ["vallon de l'Yzeron", "coteaux boisés", "centre-bourg"],
    caractere: "résidentielle verdoyante de l'ouest",
    typeBati: "maisons individuelles et résidences récentes",
    angleSerrurier: "Habitat pavillonnaire de coteau : forte demande de portes blindées, serrures haute sécurité, portails et portes de garage motorisés pour maisons familiales.",
    limitrophes: ["sainte-foy-les-lyon", "tassin-la-demi-lune", "craponne"],
  },
  {
    slug: "mions",
    nom: "Mions",
    nomComplet: "Mions",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69780",
    secteur: "Sud-est de la Métropole",
    quartiers: ["Mions centre", "lotissements résidentiels"],
    pointsDeRepere: ["centre-bourg", "secteurs pavillonnaires récents"],
    caractere: "pavillonnaire résidentielle du sud-est",
    typeBati: "maisons individuelles et lotissements",
    angleSerrurier: "Tissu très majoritairement pavillonnaire : blindage, serrures haute sécurité, motorisation de portails et portes de garage pour les maisons des lotissements.",
    limitrophes: ["saint-priest", "corbas", "venissieux"],
  },
  {
    slug: "corbas",
    nom: "Corbas",
    nomComplet: "Corbas",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69960",
    secteur: "Sud-est de la Métropole",
    quartiers: ["Corbas centre", "zone industrielle et logistique"],
    pointsDeRepere: ["grande zone logistique", "aérodrome de Corbas", "centre-bourg"],
    caractere: "pavillonnaire dotée d'une vaste zone d'activité",
    typeBati: "pavillons et importants entrepôts / locaux d'activité",
    angleSerrurier: "Fort tissu logistique et industriel (sécurisation d'entrepôts et locaux, rideaux métalliques, serrures renforcées) complété par du blindage et des portails pour les maisons du centre.",
    limitrophes: ["venissieux", "saint-priest", "mions", "feyzin"],
  },
  {
    slug: "genas",
    nom: "Genas",
    nomComplet: "Genas",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69740",
    secteur: "Est de la Métropole",
    quartiers: ["Genas centre", "Azieu", "zone industrielle Mi-Plaine"],
    pointsDeRepere: ["parc d'activités de Mi-Plaine", "centre-bourg", "axe vers l'aéroport"],
    caractere: "pavillonnaire bordée d'une très grande zone d'activité",
    typeBati: "maisons individuelles et vastes zones industrielles",
    angleSerrurier: "L'une des plus grandes zones d'activité de l'agglomération (sécurisation d'entreprises, rideaux métalliques, contrôles d'accès) aux côtés d'un habitat pavillonnaire à protéger par du blindage et des portails.",
    limitrophes: ["saint-priest", "bron", "meyzieu", "decines-charpieu"],
  },
  {
    slug: "charbonnieres-les-bains",
    nom: "Charbonnières-les-Bains",
    nomComplet: "Charbonnières-les-Bains",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69260",
    secteur: "Ouest de Lyon",
    quartiers: ["Charbonnières centre", "le Bourg"],
    pointsDeRepere: ["casino Le Lyon Vert", "hippodrome", "quartiers résidentiels boisés"],
    caractere: "huppée de l'ouest lyonnais",
    typeBati: "villas et propriétés de standing",
    angleSerrurier: "Propriétés de standing exigeantes en sécurité : portes blindées, serrures certifiées A2P, coffres, portails et sécurisation périmétrique, avec un travail soigné attendu.",
    limitrophes: ["ecully", "tassin-la-demi-lune", "dardilly"],
  },
  {
    slug: "dardilly",
    nom: "Dardilly",
    nomComplet: "Dardilly",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69570",
    secteur: "Nord-ouest de Lyon",
    quartiers: ["Dardilly centre", "Le Paisy", "parc d'activités Techlid"],
    pointsDeRepere: ["parc d'activités Techlid", "centre-bourg résidentiel"],
    caractere: "résidentielle aisée doublée d'un pôle tertiaire",
    typeBati: "maisons individuelles et bureaux / locaux d'activité",
    angleSerrurier: "Habitat pavillonnaire (blindage, portails, serrures haute sécurité) et sécurisation des entreprises du parc d'activités Techlid (contrôles d'accès, serrures renforcées).",
    limitrophes: ["ecully", "charbonnieres-les-bains", "tassin-la-demi-lune"],
  },
  {
    slug: "craponne",
    nom: "Craponne",
    nomComplet: "Craponne",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69290",
    secteur: "Ouest de Lyon",
    quartiers: ["Craponne centre", "Le Mathias"],
    pointsDeRepere: ["vestiges de l'aqueduc romain du Gier", "centre-bourg"],
    caractere: "résidentielle familiale de l'ouest",
    typeBati: "maisons individuelles et résidences récentes",
    angleSerrurier: "Habitat familial pavillonnaire : demande soutenue de blindage, de serrures haute sécurité et de motorisation de portails et portes de garage.",
    limitrophes: ["francheville", "tassin-la-demi-lune"],
  },
  {
    slug: "la-mulatiere",
    nom: "La Mulatière",
    nomComplet: "La Mulatière",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69350",
    secteur: "Sud immédiat de Lyon, au confluent",
    quartiers: ["La Mulatière centre", "Le Fort", "coteaux"],
    pointsDeRepere: ["confluent Rhône-Saône", "aquarium du Grand Lyon", "coteaux"],
    caractere: "dense sur les coteaux, au sud de Lyon",
    typeBati: "immeubles anciens de coteau et résidences",
    angleSerrurier: "Habitat dense de coteau : dépannages d'urgence fréquents en copropriété et remplacement de serrures dans l'ancien, avec quelques pavillons à sécuriser sur les hauteurs.",
    limitrophes: ["oullins", "sainte-foy-les-lyon", "lyon-2", "lyon-7"],
  },
  {
    slug: "pierre-benite",
    nom: "Pierre-Bénite",
    nomComplet: "Pierre-Bénite",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69310",
    secteur: "Sud de Lyon, rive droite du Rhône",
    quartiers: ["Pierre-Bénite centre", "Haute-Roche"],
    pointsDeRepere: ["centre hospitalier", "bords du Rhône", "vallée de la chimie"],
    caractere: "du sud mêlant résidentiel et industriel",
    typeBati: "habitat collectif, pavillons et zones d'activité",
    angleSerrurier: "Mix d'habitat collectif (dépannages d'urgence) et de sites d'activité (rideaux métalliques, sécurisation de locaux), avec du blindage pour les secteurs pavillonnaires.",
    limitrophes: ["oullins", "saint-genis-laval", "la-mulatiere"],
  },
  {
    slug: "feyzin",
    nom: "Feyzin",
    nomComplet: "Feyzin",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69320",
    secteur: "Sud-est de la Métropole, le long du Rhône",
    quartiers: ["Feyzin centre", "Les Razes", "plateau"],
    pointsDeRepere: ["raffinerie de Feyzin", "bords du Rhône", "plateau résidentiel"],
    caractere: "du sud alliant plateau pavillonnaire et site industriel majeur",
    typeBati: "pavillons sur le plateau et tissu industriel",
    angleSerrurier: "Plateau pavillonnaire à protéger (blindage, portails, portes de garage) et important tissu industriel à sécuriser (rideaux métalliques, serrures renforcées).",
    limitrophes: ["venissieux", "corbas", "saint-fons"],
  },

  // ─────────────────────────────  EST LYONNAIS / NORD-ISÈRE (hors métropole)  ─────────────────────────────
  {
    slug: "chassieu",
    nom: "Chassieu",
    nomComplet: "Chassieu",
    type: "commune",
    secteurGeo: "grand-lyon",
    codePostal: "69680",
    secteur: "Est de Lyon",
    quartiers: ["Centre-ville", "Le Châtenay", "Zone Mi-Plaine"],
    pointsDeRepere: ["Eurexpo", "Golf de Chassieu", "Zone industrielle Mi-Plaine"],
    caractere: "très dynamique, partagée entre pôle événementiel/industriel et zones résidentielles aisées",
    typeBati: "villas haut de gamme, lotissements cossus et vastes complexes industriels/commerciaux",
    angleSerrurier: "Sécurisation haut de gamme pour les villas (portes blindées sur-mesure, coffres-forts, serrures connectées) et contrats de maintenance des accès professionnels sur la zone Mi-Plaine.",
    limitrophes: ["bron", "meyzieu", "genas", "saint-priest", "decines-charpieu"],
  },
  {
    slug: "saint-quentin-fallavier",
    nom: "St-Quentin-Fallavier",
    nomComplet: "Saint-Quentin-Fallavier",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "38070",
    secteur: "Nord-Isère",
    quartiers: ["Centre-bourg", "Tharabie", "Chesnes (Parc d'activité)"],
    pointsDeRepere: ["Parc logistique de Chesnes", "Château de Fallavier", "Étang de Fallavier"],
    caractere: "pôle logistique européen majeur avec un bourg historique et rural",
    typeBati: "immenses entrepôts logistiques, bâtiments industriels, et habitat pavillonnaire",
    angleSerrurier: "Forte demande B2B pour la zone de Chesnes : contrôle d'accès, serrures industrielles, portes de secours, barrières et rideaux métalliques. Sécurisation résidentielle classique pour le bourg.",
    limitrophes: ["heyrieux"],
  },
  {
    slug: "heyrieux",
    nom: "Heyrieux",
    nomComplet: "Heyrieux",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "38540",
    secteur: "Nord-Isère / Collines",
    quartiers: ["Centre-bourg", "Les collines", "Périphérie rurale"],
    pointsDeRepere: ["Mairie d'Heyrieux", "Place des Halles", "Zones agricoles environnantes"],
    caractere: "village authentique en pleine expansion périurbaine",
    typeBati: "anciennes fermes rénovées, maisons de village de caractère et nouveaux lotissements",
    angleSerrurier: "Mise aux normes de sécurité pour les maisons neuves (portails motorisés, portes de garage) et rénovation des systèmes de fermeture sur les habitations plus anciennes. Dépannage de proximité pour les particuliers.",
    limitrophes: ["saint-quentin-fallavier", "diemoz"],
  },
  {
    slug: "saint-bonnet-de-mure",
    nom: "St-Bonnet-de-Mure",
    nomComplet: "Saint-Bonnet-de-Mure",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "69720",
    secteur: "Est / Nord-Isère",
    quartiers: ["Centre-bourg", "La Plaine", "Route Nationale"],
    pointsDeRepere: ["Mairie de St-Bonnet", "Proximité Aéroport Lyon-Saint Exupéry"],
    caractere: "périurbain attractif, partagé entre zones d'activités et quartiers très résidentiels",
    typeBati: "vaste habitat pavillonnaire et parcs d'activités tertiaires/logistiques",
    angleSerrurier: "Renforcement des accès résidentiels (serrures multipoints, blindage) face aux risques de cambriolages périurbains, et contrôle d'accès pour les entreprises locales.",
    limitrophes: ["saint-laurent-de-mure", "genas", "saint-priest", "toussieu"],
  },
  {
    slug: "saint-laurent-de-mure",
    nom: "St-Laurent-de-Mure",
    nomComplet: "Saint-Laurent-de-Mure",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "69720",
    secteur: "Est / Nord-Isère",
    quartiers: ["Centre-ville", "Le Bois", "Zone d'Activité Les Marches du Rhône"],
    pointsDeRepere: ["ZI Les Marches du Rhône", "Château de Saint-Laurent"],
    caractere: "ville charnière dynamique avec une forte composante économique",
    typeBati: "maisons individuelles récentes, noyaux anciens et entrepôts",
    angleSerrurier: "Forte demande B2B sur la ZI (portes de secours, serrures industrielles, organigrammes de clés) couplée à du dépannage de proximité pour les particuliers.",
    limitrophes: ["saint-bonnet-de-mure", "saint-pierre-de-chandieu"],
  },
  {
    slug: "saint-pierre-de-chandieu",
    nom: "St-Pierre-de-Chandieu",
    nomComplet: "Saint-Pierre-de-Chandieu",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "69780",
    secteur: "Est / Nord-Isère",
    quartiers: ["Centre", "Lafayette", "Les Hauteurs"],
    pointsDeRepere: ["Château de Chandieu", "Espaces forestiers", "Étang"],
    caractere: "villageois et verdoyant, en forte expansion pavillonnaire",
    typeBati: "essentiellement des maisons individuelles et de grandes parcelles",
    angleSerrurier: "Sécurisation périmétrique (serrures de portails, portes de garage, barillets de haute sécurité) pour prévenir les intrusions dans un secteur pavillonnaire prisé.",
    limitrophes: ["toussieu", "mions", "saint-laurent-de-mure", "chaponnay", "heyrieux"],
  },
  {
    slug: "diemoz",
    nom: "Diémoz",
    nomComplet: "Diémoz",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "38790",
    secteur: "Est / Nord-Isère",
    quartiers: ["Bourg", "Lafay", "Le Château"],
    pointsDeRepere: ["Collines nord-iséroises", "Centre village", "L'église de Diémoz"],
    caractere: "ruralité en pleine transformation vers le résidentiel",
    typeBati: "anciennes bâtisses dauphinoises, corps de ferme rénovés et constructions neuves",
    angleSerrurier: "Mise à niveau sécuritaire d'anciennes menuiseries (adaptation de serrures modernes sur vieilles portes) et sécurisation standard des nouveaux lotissements.",
    limitrophes: ["heyrieux"],
  },
  {
    slug: "toussieu",
    nom: "Toussieu",
    nomComplet: "Toussieu",
    type: "commune",
    secteurGeo: "est-nord-isere",
    codePostal: "69780",
    secteur: "Est / Nord-Isère",
    quartiers: ["Village", "Les Vignes", "La Plaine"],
    pointsDeRepere: ["Gare de Toussieu-Chandieu", "Espaces naturels agricoles"],
    caractere: "petit village paisible de l'Est lyonnais",
    typeBati: "habitat très majoritairement pavillonnaire, maisons de lotissement",
    angleSerrurier: "Interventions rapides pour les ouvertures de portes claquées/fermées et installation de serrures A2P pour protéger un habitat résidentiel souvent inoccupé en journée.",
    limitrophes: ["saint-pierre-de-chandieu", "mions", "saint-bonnet-de-mure", "chaponnay"],
  },

  // ─────────────────────────────  SUD LYONNAIS (hors métropole)  ─────────────────────────────
  {
    slug: "solaize",
    nom: "Solaize",
    nomComplet: "Solaize",
    type: "commune",
    secteurGeo: "sud-lyonnais",
    codePostal: "69360",
    secteur: "Sud lyonnais",
    quartiers: ["Centre", "Les Rues Basses", "Le Chantre"],
    pointsDeRepere: ["Vallée de la Chimie", "Institut Français du Pétrole (IFPEN)", "Bords du Rhône"],
    caractere: "résidentiel aisé et stratégique aux portes de l'industrie lyonnaise",
    typeBati: "villas de standing, grands terrains et anciens murs en pisé",
    angleSerrurier: "Besoins en serrurerie haut de gamme (cylindres haute sécurité, portes d'entrée blindées) et forte exigence sur les finitions pour un parc immobilier de valeur.",
    limitrophes: ["feyzin", "ternay"],
  },
  {
    slug: "communay",
    nom: "Communay",
    nomComplet: "Communay",
    type: "commune",
    secteurGeo: "sud-lyonnais",
    codePostal: "69360",
    secteur: "Sud lyonnais",
    quartiers: ["Bourg", "La Plaine", "Les Hauteurs"],
    pointsDeRepere: ["Carrefour autoroutier A46/A47", "Les coteaux"],
    caractere: "commune vallonnée, paisible et très accessible",
    typeBati: "maisons individuelles dominantes, peu d'habitat collectif",
    angleSerrurier: "Dépannage de proximité pour particuliers, remplacement de serrures suite à la perte de clés et sécurisation de portes de garage.",
    limitrophes: ["ternay", "simandres", "seyssuel", "chasse-sur-rhone"],
  },
  {
    slug: "ternay",
    nom: "Ternay",
    nomComplet: "Ternay",
    type: "commune",
    secteurGeo: "sud-lyonnais",
    codePostal: "69360",
    secteur: "Sud lyonnais",
    quartiers: ["Village", "Flévieu", "Les Landes"],
    pointsDeRepere: ["Château de Ternay", "Église romane", "Le Rhône"],
    caractere: "patrimonial, résidentiel et idéalement situé près des axes routiers",
    typeBati: "maisons bourgeoises, propriétés en pierres et lotissements récents",
    angleSerrurier: "Interventions minutieuses sur fermetures de maisons anciennes, pose de cornières anti-pinces et verrous de haute sûreté.",
    limitrophes: ["communay", "solaize", "chasse-sur-rhone"],
  },
  {
    slug: "chaponnay",
    nom: "Chaponnay",
    nomComplet: "Chaponnay",
    type: "commune",
    secteurGeo: "sud-lyonnais",
    codePostal: "69970",
    secteur: "Sud lyonnais",
    quartiers: ["Centre-village", "Parc d'Activité Chapotin", "Les Pépinières"],
    pointsDeRepere: ["Zone d'Activité Chapotin", "Vallon de l'Ozon"],
    caractere: "très attractif, parfait équilibre entre pôle économique majeur et village résidentiel",
    typeBati: "pépinières d'entreprises, locaux tertiaires et belles villas",
    angleSerrurier: "Contrats de maintenance et de sécurisation pour le parc économique Chapotin (ferme-portes, portes coupe-feu, barres antipanique), en plus du résidentiel.",
    limitrophes: ["marennes", "corbas", "mions", "toussieu", "saint-pierre-de-chandieu"],
  },
  {
    slug: "marennes",
    nom: "Marennes",
    nomComplet: "Marennes",
    type: "commune",
    secteurGeo: "sud-lyonnais",
    codePostal: "69970",
    secteur: "Sud lyonnais",
    quartiers: ["Bourg", "Les Hauteurs", "La Plaine"],
    pointsDeRepere: ["Domaine de Marennes", "Espaces agricoles"],
    caractere: "petit village de charme, très tranquille et cossu",
    typeBati: "propriétés bourgeoises, fermes restaurées et maisons individuelles",
    angleSerrurier: "Modernisation des équipements de sécurité sur des bâtisses de valeur, motorisation d'accès et réparations suite à des tentatives d'effraction.",
    limitrophes: ["chaponnay", "simandres", "corbas"],
  },
  {
    slug: "simandres",
    nom: "Simandres",
    nomComplet: "Simandres",
    type: "commune",
    secteurGeo: "sud-lyonnais",
    codePostal: "69360",
    secteur: "Sud lyonnais",
    quartiers: ["Village", "La Plaine"],
    pointsDeRepere: ["Espaces naturels", "Château de Simandres", "Les sentiers de l'Ozon"],
    caractere: "rural, naturel et protégé des grandes nuisances urbaines",
    typeBati: "maisons dauphinoises traditionnelles et habitat résidentiel espacé",
    angleSerrurier: "Mise en sécurité de propriétés à l'écart du flux urbain, installations de serrures 3 à 5 points et d'équipements dissuasifs.",
    limitrophes: ["marennes", "communay"],
  },

  // ─────────────────────────────  SECTEUR DE VIENNE (hors métropole)  ─────────────────────────────
  {
    slug: "vienne",
    nom: "Vienne",
    nomComplet: "Vienne",
    type: "commune",
    secteurGeo: "secteur-vienne",
    codePostal: "38200",
    secteur: "Isère Rhodanienne / Sud de Lyon",
    quartiers: ["Centre ancien", "Estressin", "Vallée de Gère", "Malissol", "L'Isle"],
    pointsDeRepere: ["Théâtre Antique", "Temple d'Auguste et de Livie", "Gare de Vienne", "Les quais du Rhône"],
    caractere: "historique, dense en centre-ville et résidentiel sur les hauteurs",
    typeBati: "bâtiments anciens protégés en centre, maisons individuelles en périphérie et petits immeubles",
    angleSerrurier: "Interventions délicates sur portes anciennes en centre-ville historique, et forte demande de sécurisation standard (blindage, alarmes, portes de garage) pour les zones pavillonnaires sur les collines.",
    limitrophes: ["seyssuel"],
  },
  {
    slug: "chasse-sur-rhone",
    nom: "Chasse-sur-Rhône",
    nomComplet: "Chasse-sur-Rhône",
    type: "commune",
    secteurGeo: "secteur-vienne",
    codePostal: "38670",
    secteur: "Secteur Vienne",
    quartiers: ["Centre-ville", "Les Hauts de Chasse", "Zone Commerciale Chasse Sud"],
    pointsDeRepere: ["Centre commercial Chasse Sud", "Gare de Chasse-sur-Rhône", "Bords du Rhône"],
    caractere: "carrefour commercial très dynamique et résidentiel sur les hauteurs",
    typeBati: "pôle commercial majeur, immeubles de centre-ville et forte densité pavillonnaire",
    angleSerrurier: "Dépannage d'urgence et rideaux métalliques/vitrines pour la grande zone commerciale, et sécurisation standard anti-effraction (portes, cylindres) pour les lotissements sur les hauteurs.",
    limitrophes: ["givors", "seyssuel", "ternay", "communay", "vienne"],
  },
  {
    slug: "seyssuel",
    nom: "Seyssuel",
    nomComplet: "Seyssuel",
    type: "commune",
    secteurGeo: "secteur-vienne",
    codePostal: "38200",
    secteur: "Secteur Vienne",
    quartiers: ["Le Village", "Les Vignobles", "Hauteurs de Seyssuel"],
    pointsDeRepere: ["Vignobles de Seyssuel", "Ruines du Château", "Église Saint-Martin"],
    caractere: "résidentiel, calme et viticole, très prisé",
    typeBati: "villas haut de gamme, propriétés isolées et maisons de caractère",
    angleSerrurier: "Clientèle exigeante nécessitant des installations de haute sécurité : portes blindées sur-mesure, alarmes, coffres-forts et motorisation de grands portails pour propriétés isolées.",
    limitrophes: ["vienne", "chasse-sur-rhone", "communay"],
  },
];

// ─────────────────────────────  HELPERS  ─────────────────────────────

/** Préfixe d'URL commun aux pages de zone. */
export const ZONE_URL_PREFIX = "/zones/serrurier-";

/** URL complète d'une zone (ex: /zones/serrurier-lyon-3). */
export const zoneHref = (z: Pick<Zone, "slug">) => `${ZONE_URL_PREFIX}${z.slug}`;

/** Tous les slugs — pratique pour generateStaticParams(). */
export const allZoneSlugs = zones.map((z) => z.slug);

/** Récupère une zone par son slug. */
export const getZoneBySlug = (slug: string): Zone | undefined =>
  zones.find((z) => z.slug === slug);

/** Zones filtrées par type. */
export const arrondissements = zones.filter((z) => z.type === "arrondissement");
export const communes = zones.filter((z) => z.type === "commune");

/** Communes réellement membres de la Métropole de Lyon (sous-ensemble de `communes`). */
export const communesGrandLyon = communes.filter((z) => z.secteurGeo === "grand-lyon");

/** Libellés honnêtes par secteur géographique, pour le hub /zones et les regroupements. */
export const SECTEUR_GEO_LABELS: Record<SecteurGeo, string> = {
  lyon: "Lyon (arrondissements)",
  "grand-lyon": "Grand Lyon",
  "est-nord-isere": "Est lyonnais et Nord-Isère",
  "sud-lyonnais": "Sud lyonnais",
  "secteur-vienne": "Secteur de Vienne",
};

/** Ordre d'affichage des secteurs géographiques (du plus proche au plus éloigné du siège). */
export const SECTEUR_GEO_ORDER: SecteurGeo[] = [
  "lyon",
  "grand-lyon",
  "est-nord-isere",
  "sud-lyonnais",
  "secteur-vienne",
];

/** Zones d'un secteur géographique donné. */
export const zonesBySecteurGeo = (secteur: SecteurGeo): Zone[] =>
  zones.filter((z) => z.secteurGeo === secteur);

/** Zones limitrophes hydratées (pour le maillage interne d'une page). */
export const getLimitrophes = (slug: string): Zone[] => {
  const zone = getZoneBySlug(slug);
  if (!zone) return [];
  return zone.limitrophes
    .map(getZoneBySlug)
    .filter((z): z is Zone => Boolean(z));
};
